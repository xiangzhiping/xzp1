<template>
  <div class="captcha_login_card">
    <el-form :model="userAccountRegisterForm" :rules="rules" ref="formRef">
      <el-form-item prop="account" class="item_form">
        <div class="login_ele">
          ...
          <div class="email_input">
            <el-input v-model="userAccountRegisterForm.account" placeholder="请输入邮箱号"></el-input>
          </div>
        </div>
      </el-form-item>
      <el-form-item prop="account" class="item_form">
        <div class="login_ele">
          ...
          <div class="email_input">
            <el-input v-model="userAccountRegisterForm.password" placeholder="请输入密码"></el-input>
          </div>
        </div>
      </el-form-item>
        ...
        <div class="login_ele">
          <div class="captcha_input">
            ...
            <div class="email_captcha_input">
              <el-input v-model="userAccountRegisterForm.captcha" placeholder="请输入邮箱验证码"></el-input>
            </div>
          </div>
      ...
      <div class="login_ele">
        <el-button type="primary" class="login_button" @click="userAccountRegisterHandel">注 册</el-button>
      </div>
    </el-form>
  </div>
</template>
...
import {userAccountRegister, identityCaptchaSend} from '@/apis/login.js';
import {EmailIcon, EmailCaptchaIcon, PasswordIcon} from "@/components/icons/index.js";

const formRef = ref(null);
const userAccountRegisterForm = reactive({
  account: null,
  password:null,
  captcha: null,
  type: 'email'
...
const captchaSendForm = reactive({
  account: userAccountRegisterForm.account,
  scene: 'register',
  type: userAccountRegisterForm.type
});

watch(
    () => userAccountRegisterForm.account,
    (newValue) => {
...
const userAccountRegisterHandel = async () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      const res = await userAccountRegister(userAccountRegisterForm)
      console.log(res)
      if (res.code !== 200) {
        await stopCountdown();
      }
